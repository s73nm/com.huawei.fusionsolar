...
// Only read the inverter's accumulated yield register (32106)
const INVERTER_YIELD_REGISTERS = {
...
const REQUIRED_CAPABILITIES = [
  ...
  'meter_power.imported',   // grid accumulated / imported energy (kWh)
  'meter_power.exported',   // grid exported energy (kWh)
  'meter_power',            // calculated house consumption: inverter yield + grid import - grid export (kWh)
...
class DTSU666ModbusTestDevice extends Device {

  async onInit() {
    this.log(`[TEST] Device initialised: ${this.getName()}`);
  ...
  async _fetchAndUpdate() {
    ...
    try {
      ...
      const gridImport = meter.gridAccumulatedEnergy ?? null;
      const gridExport = meter.gridExportedEnergy    ?? null;
      await this._set('meter_power.imported', gridImport);
      await this._set('meter_power.exported', gridExport);

      // ── Hausverbrauch gesamt ──────────────────────────────────────────────
      // Formula: inverter total yield + grid import - grid export
      // All values in kWh (cumulative). Result = total energy consumed by the house.
      try {
        const inv = await readModbusRegisters(address, port, modbusId, INVERTER_YIELD_REGISTERS);
        const invYield = inv.accumulatedYieldEnergy ?? null;
        if (invYield !== null && gridImport !== null && gridExport !== null) {
          const houseConsumption = Math.round((invYield + gridImport - gridExport) * 100) / 100;
          await this._set('meter_power', houseConsumption);
          this.log(`[TEST] Hausverbrauch: ${invYield} + ${gridImport} - ${gridExport} = ${houseConsumption} kWh`);
        }
      } catch (err) {
        this.log('[TEST] Inverter yield read skipped:', err.message);
      }
...
module.exports = DTSU666ModbusTestDevice;
